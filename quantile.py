...
x_mean=summary.loc['mean']/np.sqrt((1/252))
x_std=summary.loc['std']/np.sqrt((1/252))
...
plt.plot(summary.columns,x_mean*100,linewidth=1)
plt.plot(summary.columns, x_std*100,linewidth=1)
last_quantile=data['quantile_gamma'].iloc[-1]

plt.vlines(x=last_quantile, ymin=min(x_mean)*100 ,ymax=max(x_std)*100, colors='purple', ls='--', lw=0.5)
fig2.autofmt_xdate()
